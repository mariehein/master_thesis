...
def bump_hunt(folder, name, cwola):
	...
	for k in range(9):
		...
		for i in range(5): 
			...
			length = np.size(np.where(Y_test[:,1]==0))
			...
			for j, perc in enumerate(BH_percentiles):
				...
				if perc==0:
					eps=0.
				N_samples_after[j,i] = np.size(np.where(samples_preds>eps))
				N_samples[j,i] =len(samples_preds)
				N_after[j,i] = np.size(np.where(inner_preds>eps))
				N[j,i] =len(inner_preds)
				N_bkg[j,i] = np.size(np.where(bkg_preds>eps))
				N_sig[j,i] = np.size(np.where(sig_preds>eps))
...
bump_hunt("results/k_fold/cathode_sliding_without/", "cathode_without", False)
bump_hunt("results/k_fold/cathode_sliding/", "cathode", False)
bump_hunt("results/k_fold/cathode_sliding_04/", "cathode_04", False)
bump_hunt("results/k_fold/cathode_sliding_03/", "cathode_03", False)

bump_hunt("results/k_fold/cwola_sliding_without/", "cwola_without", True)
bump_hunt("results/k_fold/cwola_sliding/", "cwola", True)
bump_hunt("results/k_fold/cwola_sliding_10/", "cwola_10", True)
bump_hunt("results/k_fold/cwola_sliding_03/", "cwola_03", True)
