...
def plot_fn(tpr,fpr,color,label, background):
	if background:
		tpr_mean, fpr_mean, fpr_min, fpr_max= mean_and_percentiles(tpr,fpr)
		plt.plot(tpr_mean, fpr_mean, color=color, linestyle='--',alpha=0.5)
		return
	if envelope: 
...
def import_and_plot(path, label=None, color="grey", folder="rocs/", N_runs =10, start_runs =0, background= False):	
	if label is None: 
		label=path
	...
	plot_fn(tpr,fpr,color,label,background)
	return tpr,fpr
...
def plot_end(name, ylim=1e5, small=True): 
	...
	if envelope:
		...
		plt.savefig("plots/combined/6inputs/roc_"+name+".pdf")
	plt.show()
...
if small:
	...
	plt.figure()
import_and_plot("roc_IAD_averaging", label="IAD",color="grey",folder="results/4_default/IAD/", background=True)
import_and_plot("roc_cwola_averaging", label="CWoLa",color="orange",folder="results/4_default/cwola/", background=True)
import_and_plot("roc_supervised_averaging", label="Supervised",color="green",folder="results/4_default/supervised/", background=True)
import_and_plot("roc_cathode_averaging", label="CATHODE", color="deeppink", folder="results/cathode/cathode_RealNVP_avg/", background=True)
plot_end(name)
"""
...
import_and_plot("roc_supervised_averaging", label="0G, reduced statistics", color="C6", folder="results/supervised_normal_signal/0G/")
import_and_plot("roc_supervised_averaging", label="10G, reduced statistics", color="C5", folder="results/supervised_normal_signal/10G/")
name = "gaussian_supervised"
plot_end(name, small=small)
